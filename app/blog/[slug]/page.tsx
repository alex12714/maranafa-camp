  ...
  return {
    ...
    openGraph: {
      ...
      images: article.coverUrl
        ? [{ url: article.coverUrl, width: 1200, height: 630, alt: article.title }]
        : [{ url: `${SITE_URL}/images/maranafa-og.jpg`, width: 1200, height: 630, alt: "Маранафа" }],
    },
    twitter: {
...
function ShareButtons({ url, title }: { url: string; title: string }) {
  ...
  const shares = [
    ...
    {
      label: "Facebook",
      color: "bg-[#1877f2] hover:bg-[#0d65d1]",
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {shares.map((s) => (
        <a
          ...
          title={s.label}
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-white text-xs font-medium transition-colors ${s.color}`}
        >
          {s.icon}
          {s.label}
        </a>
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      {/* Hero text (overlaid on image or standalone red section) */}
      {!article.coverUrl ? (
        <div className="bg-gradient-to-br from-[#B22234] to-[#7a0e1e] text-white py-16 px-4">
          <div className="container mx-auto max-w-3xl">
            ...
            {article.subtitle && <p className="text-white/80 text-lg">{article.subtitle}</p>}
          </div>
        </div>
      ) : (
      ...
      <div className="container mx-auto max-w-3xl px-4 py-10">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-8 pb-8 border-b border-gray-100">
          ...
          {article.date && <span>📅 {dateStr}</span>}
        </div>
