...
const spending = [
  ...
  {
    icon: Stethoscope,
    titleKey: "Оплата визитов к специалистам",
    descKey: "Часть взноса идёт на приёмы у врачей, юриста и логопеда — по ставкам, согласованным для участников заранее.",
  },
  {
    icon: Car,
  ...
  },
  {
    ...
    descKey: "Тарифный план Rosetto Call для участников: мы оплачиваем пакет целиком и распределяем его между участниками.",
  },
  {
    icon: HeartPulse,
    titleKey: "Медицинская страховка",
    descKey: "Взносы по групповому полису — условия для группы лучше тех, что доступны при индивидуальном обращении.",
  },
]
...
const afterAccess = [
  {
    ...
    descKey: "Доступ к машинам открывается отдельно: короткое интервью и проверка водительского удостоверения. И то, и другое проходит в приложении — загружаете права, проходите интервью, получаете доступ.",
  },
  {
    icon: Clock,
  ...
  },
  {
    ...
    descKey: "Сразу после подключения запускается перевод на тариф Rosetto Call — мы сопровождаем перенос номера, ваш номер остаётся прежним.",
  },
  {
    icon: Smartphone,
...
const faq = [
  {
    q: "Это страховка?",
    a: "Нет. Rosetto — это членство в общине с набором услуг. Медицинская страховка входит в план как одна из услуг, но сама программа страховым продуктом не является.",
  },
  {
    q: "Сколько это стоит?",
  ...
  },
  {
    ...
    a: "Да. Участие прекращается по вашему заявлению. Условия выхода прописаны в договоре участия, и вы видите их до подписания.",
  },
  {
    q: "Кто отвечает за качество специалистов?",
    a: "Мы. Партнёров подбираем и проверяем сами. Если специалист не подошёл — скажите координатору: это влияет на то, останется ли он в пуле.",
  },
  {
    q: "Что происходит с моими данными?",
    a: "Данные из формы используются только для связи по программе Rosetto. Мы не передаём их третьим лицам и удаляем по вашему запросу.",
  },
]
